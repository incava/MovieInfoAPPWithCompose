package com.incava.movieinfoappwithcompose.ui.components.dialog.components.content
...
import com.incava.movieinfoappwithcompose.ui.util.getAnnotatedText
    ...
    Column(
        ...
        Text(
            ...
            style = MaterialTheme.typography.titleMedium.copy(
                color = MaterialTheme.custumColorScheme.secondary
            ),
...
fun ColumnScope.StarRatingBar(score: Float) {
    Card(
        ...
        Box(
            ...
            val foregroundColor = MaterialTheme.custumColorScheme.primary.toArgb()
            val starBackgroundColor = MaterialTheme.custumColorScheme.background.copy(
                alpha = 0.2f
            ).toArgb()
            val ratingBackgroundColor = MaterialTheme.custumColorScheme.primaryContainer.copy(
                alpha = 0.05f
            ).toArgb()

            AndroidView(
                // RatingBar 위젯을 사용 하기 위해 AndroidView 사용
                // Fragment, Activity에서 작성하지 않아도 가져올 수 있음.
                modifier = Modifier
                    .wrapContentWidth()
...
//                update = {
//                    // 화면 주기에 따른 의존적, inflate될 때마다 업데이트가 되기 때문에 신중히 사용.
//                }
            )
        }
    }
